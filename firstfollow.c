		...
		if(productions[i][0]==c)
		{
			...
			else
			{
				...
				while(productions[i][j]!='\0')
				{
					...
					int k;
					for(k=0;subresult[k]!='\0';k++)if(subresult[k]!='#') addToResult(result,subresult[k]);
					for(k=0;subresult[k]!='\0';k++)if(subresult[k]=='#')foundEpsilon=1;
...
/*				if(c=='B')
				...
				if(productions[i][l]=='\0' && productions[i][j]!=productions[i][0])
				{
					...
					int k;
					for(k=0;subresult[k]!='\0';k++) addToResult(result,subresult[k]);
					break;
				}				
			}
			j++;
...
/*	follow(result,'B');
	...
	for(i=0;i<n;i++) visit[productions[i][0]]=1;
	printf("NonTe		first 		follow\n");
	for(i=0;i<100;i++)if(visit[i]==1)
	{
		printf("%c 		",i);
		first(result,i);
		int j;
		for(j=0;result[j]!='\0';j++)printf("%c ",result[j]);
		printf("		");
		follow(result,i);
		for(j=0;result[j]!='\0';j++)printf("%c ",result[j]);
		printf("\n");
